 ...
 * 添加一个传统的下拉刷新控件
 *
 * @param block 进入刷新状态就会自动调用这个block,在这个block里面可以拿到scrollView的contentoffset值来设置一些东西，像比如背景放大处理
 */

- (MJRefreshLegendHeader *)addLegendHeaderWithRefreshingBlock:(void (^)(CGFloat contentoffsetY))block withcompleteBlock:(void(^)())complete;
...
/** 列表向上滚动时，结束刷新状态add by Long*/
- (void)endRefreshingScrollUp;
