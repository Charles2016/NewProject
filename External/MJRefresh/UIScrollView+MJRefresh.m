...
#pragma mark - 下拉刷新
- (MJRefreshLegendHeader *)addLegendHeaderWithRefreshingBlock:(void (^)(CGFloat contentoffsetY))block
                                            withcompleteBlock:(void(^)())complete{
    MJRefreshLegendHeader *header = [[MJRefreshLegendHeader alloc] init];
    //放大block
    header.headerRefreshingBlock = block;
    // 刷新数据block
    header.refreshingBlock = complete;
    header.stateHidden = YES;
    header.updatedTimeHidden = YES;
    header.animateimageView.hidden = YES;
    [header.animateimageView removeFromSuperview];
    self.header = header;
    return header;
}
...
- (MJRefreshGifHeader *)addGifHeader
{
    ...
    //重写setImages方法，来制定固定的图片数组，播放数组动画
    [header setImages:imagesArray forState:MJRefreshHeaderStateRefreshing];
    self.header = header;
    return header;
}
