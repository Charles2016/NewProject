...
- (void)loadView
{
    ...
    // 隐藏状态栏在此处不要对状态栏进行隐藏操作，会出现高度计算问题
...
{
    NSLog(@"ThisImageIndex---%zd", ThisImageIndex );
    NSLog(@"_currentPhotoIndex---%zd", _currentPhotoIndex);
...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [SDWebImageManager downloadWithURL:photo.url completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            photo.image = image;
        }];
    }
