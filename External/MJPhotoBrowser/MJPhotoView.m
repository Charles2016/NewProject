...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        _imageView.layer.borderColor = kColorClear.CGColor;
        _imageView.tintColor = kColorClear;
...
//        _photo.srcImageView.image = nil;
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                ...
                [photoView photoDidFinishLoadWithImage:image imageURL:imageURL];
            }];
...
#pragma mark 加载完毕
- (void)photoDidFinishLoadWithImage:(UIImage *)image imageURL:(NSURL *)imageURL
{
    if (image) {
        self.scrollEnabled = YES;
        _photo.image = image;
        ...
        [self addSubview:_photoLoadingView];
        [_photoLoadingView showFailure:_photo.url.absoluteString.length ? @"加载失败" : @""];
    }
    // 设置缩放比例
    [self adjustFrame];
}
...
- (void)hide
{
    ...
    // 把keywindow放到正常level
    [DataHelper bringKeyWindowInFront:NO];
    // 移除进度条
    [_photoLoadingView removeFromSuperview];
