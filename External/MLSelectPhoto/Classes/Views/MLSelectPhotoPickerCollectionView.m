...
- (void)setDataArray:(NSArray *)dataArray{
    ...
    if (self.isRecoderSelectPicker){
        ...
        for (id photoAsset in self.selectAsstes) {
            for (id dataAsset in self.dataArray) {
                if ([photoAsset isEqual:dataAsset]) {
                    [selectAssets addObject:dataAsset];
                    break;
                }
            }
        }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        // 需要记录选中的值的数据
        if (self.isRecoderSelectPicker) {
            for (id asset in self.doneAsstes) {
                if ([[MLPhotoTool sharePhotoTool] isSameAsset:asset asset:self.dataArray[indexPath.item]]) {
        ...
        [[MLPhotoTool sharePhotoTool] getThumbImageWithAsset:self.dataArray[indexPath.item] complete:^(UIImage *thumbImage) {
            cellImgView.image = thumbImage;
        }];
    }
