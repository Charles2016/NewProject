...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets {
    ...
    [self.selectAssets addObjectsFromArray:selectPickerAssets];
...
- (void)refresh:(NSNotification *)notify {
    self.selectAssets = notify.userInfo[@"assets"];
    [self.collectionView.selectsIndexPath removeAllObjects];
    self.collectionView.selectAsstes = notify.userInfo[@"assets"];
    self.collectionView.doneAsstes = notify.userInfo[@"assets"];
...
- (void)setupAssets {
    mWeakSelf;
    [[MLPhotoTool sharePhotoTool] getAllDataWithAssetsGroup:self.photoGroup.assetsGroup sourceType:self.sourceType isAscending:NO complete:^(NSArray *groups) {
        weakSelf.collectionView.dataArray = groups;
    }];
}
...
- (void)setPhotoGroup:(MLSelectPhotoPickerGroup *)photoGroup{
    
    _photoGroup = photoGroup;
    self.title = photoGroup.title;
    // 获取Assets
    [self setupAssets];
}
...
- (void)pickerCollectionViewDidSelected:(MLSelectPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(id)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            if([[MLPhotoTool sharePhotoTool] isSameAsset:self.selectAssets[i] asset:asset]){
...
- (void)preview:(NSNotification *)notify {
    MLSelectPhotoBrowserViewController *browserVc = [[MLSelectPhotoBrowserViewController alloc] init];
    browserVc.doneStr = self.doneStr;
    ...
    if ([notify isKindOfClass:[NSNotification class]]) {
        browserVc.currentPage = [notify.userInfo[@"currentPage"] integerValue];
        browserVc.photos = self.collectionView.dataArray;
        browserVc.doneAssets = notify.userInfo[@"selectAssets"];
    } else {
