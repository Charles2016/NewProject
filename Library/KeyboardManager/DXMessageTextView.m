...
//  GoodHappiness
//
//  Created by chaolong on 16/5/16.
//  Copyright © 2016年 Charles. All rights reserved.
//

#import "DXMessageTextView.h"

@implementation DXMessageTextView
...
- (void)setPlaceHolder:(NSString *)placeHolder {
    ...
    NSUInteger maxChars = [DXMessageTextView maxCharactersPerLine];
...
- (NSUInteger)numberOfLinesOfText {
    return [DXMessageTextView numberOfLinesForMessage:self.text];
}
...
+ (NSUInteger)numberOfLinesForMessage:(NSString *)text {
    return (text.length / [DXMessageTextView maxCharactersPerLine]) + 1;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if([self.text length] == 0 && self.placeHolder) {
        CGRect placeHolderRect = CGRectMake(15.0f,
                                            7.0f,
                                            rect.size.width,
                                            rect.size.height);
