...
+ (instancetype)actionSheetWithTitle:(NSString *)title
                        ...
                        otherButtons:(NSArray *)otherButtons
                            complete:(void (^)(NSInteger buttonIndex))complete {
    HXFAlertView *alertView = [[HXFAlertView alloc] initActionSheetWithTitle:title message:message cancelButton:cancelButton otherButtons:otherButtons otherColor:nil alertViewType:AlertViewSheet complete:complete];
    return alertView;
}
...
+ (instancetype)actionSheetWithTitle:(NSString *)title
    ...
    HXFAlertView *alertView = [[HXFAlertView alloc] initActionSheetWithTitle:title message:message cancelButton:cancelButton otherButtons:otherButtons otherColor:otherColor alertViewType:alertViewType complete:complete];
    return alertView;
}
...
- (instancetype)initAlertViewWithTitle:(NSString *)title
    ...
    if (self) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self setUIWithTitle:title message:message cancelButton:cancelButton otherButtons:otherButton.length ? @[otherButton] : nil otherColor:nil alertViewType:alertViewType];
        });
    }
    return self;
}
...
- (instancetype)initActionSheetWithTitle:(NSString *)title
    ...
    if (self) {
        _completeBlock = complete;
        dispatch_async(dispatch_get_main_queue(), ^{
            [self setUIWithTitle:title message:message cancelButton:cancelButton otherButtons:otherButtons otherColor:otherColor alertViewType:alertViewType];
        });
...
- (void)setUIWithTitle:(NSString *)title
    ...
    if (alertViewType == AlertViewSingle || alertViewType == AlertViewNormal) {
        ...
        CGFloat viewHeight = ((![NSString isNull:title] && ![NSString isNull:message]) ? 60 : 49) + 49.5 * otherButtons.count + 5 + 49;
        ...
        if (alertViewType == AlertViewSheet) {
            ...
            _spanceView.backgroundColor = kColorNavBground;
        ...
        _titleLabel.frame = CGRectMake (0, 7.5, _alertView.width, [NSString isNull:message] ? 34 : 15);
...
- (void)hide {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (_alertViewType == AlertViewSheet || _alertViewType == AlertViewSheetFull) {
            self-> _alertView.frame = CGRectMake (0, self.bottom, self.width, self.height);
            if ([self->_cancelButton.superview isEqual:self]) {
                self->_cancelButton.top = self->_alertView.bottom + 5;
            }
        } else {
