...
/// 是否为有效的url
- (BOOL)isValidURL {
    NSString *regex = @"^((([hH][tT][tT][pP][sS]?|[fF][tT][pP])\\:\\/\\/)?([\\w\\.\\-]+(\\:[\\w\\.\\&%\\$\\-]+)*@)?((([^\\s\\(\\)\\<\\>\\\\\\\"\\.\\[\\]\\,@;:]+)(\\.[^\\s\\(\\)\\<\\>\\\\\\\"\\.\\[\\]\\,@;:]+)*(\\.[a-zA-Z]{2,4}))|((([01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\.){3}([01]?\\d{1,2}|2[0-4]\\d|25[0-5])))(\\b\\:(6553[0-5]|655[0-2]\\d|65[0-4]\\d{2}|6[0-4]\\d{3}|[1-5]\\d{4}|[1-9]\\d{0,3}|0)\\b)?((\\/[^\\/][\\w\\.\\,\\?\\\'\\\\\\/\\+&%\\$#\\=~_\\-@]*)*[^\\.\\,\\?\\\"\\\'\\(\\)\\[\\]!;<>{}\\s\\x7F-\\xFF])?)$";
...
- (BOOL)isHXAccount {
    NSString *rexPrefix = @"^[a-zA-Z]+";
    NSString *rexStr = @"^[0-9a-zA-Z_\\-]{6,20}$";
    BOOL prefix = [self isMatchedByRegex:rexPrefix];
    BOOL rex = [self isMatchedByRegex:rexStr];
    return (prefix && rex);
}

/// 有效的昵称，只包含中文，英文，数字，下划线
- (BOOL)isLegalNickname {
    NSString *rexString = @"^[\\u4e00-\\u9fa5\\w]*$";
    BOOL isMatch = [self isMatchedByRegex:rexString];
    return isMatch;
}

// 字母/数字/特殊字符组成，6-12位,必须由字母、数字、特殊字符中任意两组组成
- (BOOL)isHxPassword {
    if ([self isMatchedByRegex:@"^\\S{6,12}$"]) {
        int flagCount = 0;
        if ([self isMatchedByRegex:@"\\d+"]) {
            flagCount++;
        }
        if ([self isMatchedByRegex:@"[a-zA-Z]+"]) {
            flagCount++;
        }
...
- (BOOL)isHxPhoneNumber {
    return [self isMatchedByRegex:@"^[0-9]{6,11}$"];
}
